  ...
  return (
    <div className="w-full max-w-5xl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-display text-xl font-semibold text-foreground">Akun Pengguna</h1>
              ...
              {editingId === u.id ? (
                ...
                <div>
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-sm font-medium text-foreground">{u.nama_lengkap}</p>
            ...
            <tbody className="divide-y divide-border">
              {(users ?? []).map((u) => (
                <tr key={u.id}>
                  {editingId === u.id ? (
                    <>
                      <td className="px-4 py-2 text-xs text-muted-foreground">{u.nip}</td>
                      <td className="px-4 py-2"><Input value={editForm.nama} onChange={(e) => setEditForm({ ...editForm, nama: e.target.value })} className="h-8 text-sm" /></td>
                    ...
                    <>
                      ...
                      <td className="px-4 py-3 text-muted-foreground">{u.instansi_nama?.nama_instansi ?? "—"}</td>
                      <td className="px-4 py-3">
                        <div className="flex gap-1">
                          <button onClick={() => { setEditingId(u.id); setEditForm({ nama: u.nama_lengkap, role: u.role, instansi_id: u.instansi_id ?? "" }); }}
                            className="rounded p-1 text-muted-foreground hover:bg-muted"><Pencil size={14} /></button>
